const encodedMapping = ""; 

const customMapping = JSON.parse(atob(encodedMapping))

const encode = t => [...t].map(c => customMapping[c] ?? c).join('')

const reverse = Object.fromEntries(
Object.entries(customMapping).map(([k,v]) => [v,k])
)

const decode = t => {
let out = '', buf = ''
for (let c of t) {
buf += c
if (reverse[buf]) {
out += reverse[buf]
buf = ''
}
}
return out
}

const encInput = document.getElementById("encode-text")
const encOut = document.getElementById("encoded-text")
const decInput = document.getElementById("decode-text")
const decOut = document.getElementById("decoded-text")

encInput.oninput = () => encOut.value = encode(encInput.value)
decInput.oninput = () => decOut.value = decode(decInput.value)

document.getElementById("copy").onclick = () => {
navigator.clipboard.writeText(encOut.value)
}
